...
- **Decorative background layers are opacity-controlled, never hardcoded.** A campaign that adds an image layer behind the boards (a doodle pattern, a texture) exposes a `doodle-opacity` range (0-100) in the Background group, applied as `--doodle-opacity` on `.board` and read by the layer's own `opacity: var(--doodle-opacity, .6)`. Pair it with a per-scene "hide on this board" checkbox (`scene-hide-background`) so one board can opt out without discarding the setting for the rest of the campaign. Never bake a fixed opacity, or a decorative shape with no purpose (a bare circle, a blob), into board CSS without exposing a control for it — a later adjustment should happen from the sidebar, not by editing CSS by hand.
- **Text highlighter is optional, per key, three states.** When a campaign wants a marker/highlighter effect behind the headline or subhead, wrap that element's rendered text in one inline `<mark class="hl">`, set `box-decoration-break: clone` so wrapped text gets one band per visual line (not one rectangle spanning the whole block), and blend with `mix-blend-mode: multiply` so any chosen color reads like real ink over the board rather than an opaque sticker. Offer exactly three styles per key — `none` (default, no wrapper styling), `straight` (a clean rectangle) and `brush` (the same rectangle with a fixed, small, horizontally-tiling rough-edge SVG `mask-image` so it reads like a marker pen, never a randomly generated shape) — plus a 10–100 thickness range and a color field, both editable from the sidebar, independently for `headline` and `subhead`, and shared across locales like the rest of `TYPE_VISUAL_FIELDS`.
